...
molecular_surface::molecular_surface(){	
...
void molecular_surface::initialize_grid(){
...
void molecular_surface::initialize_molecular_surface(){
...
void molecular_surface::construct_saddle_each_torus(TorusIter current_torus){
...
void molecular_surface::construct_saddle_faces(){
...
void molecular_surface::check_duplex_concave(){
	for (int i=0;i<m_torus_vec.size();i++){
		TorusIter current_torus = m_torus_vec[i];
...
void molecular_surface::store_duplex_concaves(Concave_SphereIter current_concave,std::vector<Concave_SphereIter> &full_duplex_concaves){
...
void molecular_surface::construct_torus(){
...
bool molecular_surface::is_neighbor_atom(AtomIter atom_i, AtomIter atom_j){
	double r_i = atom_i->r;
	double r_j = atom_j->r;
...
void molecular_surface::mark_torus(){
	for(int t_id=0;t_id<m_torus_vec.size();t_id++){
		TorusIter current_torus = m_torus_vec[t_id];
...
void molecular_surface::collision_detection(CVector3d probe_1,CVector3d probe_2,AtomIter atom_i,AtomIter atom_j, AtomIter atom_k,bool &is_collide_1, bool &is_collide_2){
...
void molecular_surface::construct_single_concave(CVector3d probe_center,AtomIter atom_i,AtomIter atom_j,AtomIter atom_k,TorusIter torus_ij,TorusIter torus_jk, TorusIter torus_ki){
	double r_i  = atom_i->r;
	double r_j  = atom_j->r;
	double r_k = atom_k->r;
...
void molecular_surface::probe_placement(){
	for(int t_id=0;t_id<m_torus_vec.size();t_id++){
		TorusIter current_torus = m_torus_vec[t_id];
...
bool molecular_surface::is_inside_atom(AtomIter current_atom,CVector3d grid_loc){
...
bool molecular_surface::is_inside_concave_sphere(CVector3d grid_pos,CVector3d sphere_center){
...
bool molecular_surface::is_inside_tetrahedron(CVector3d p1,CVector3d p2, CVector3d p3, CVector3d p4, CVector3d test_point){
...
bool molecular_surface::is_f_InsideTorus_positive(double x,double y,double z,double R,double r){
...
bool molecular_surface::is_f_quartic_equation_positive(double x,double y,double z,double R,double r){
...
bool molecular_surface::is_located_in_selfintersecting_part(double x,double y,double z,double R,double r){
...
bool molecular_surface::is_located_within_lemon_part(double x,double y,double z,double R,double r){
...
bool molecular_surface::is_located_on_lemon_part(double x,double y,double z,double R,double r){
	//current torus is a donut torus
	if(R>r)
		return false;
...
bool molecular_surface::is_covered_by_saddles(CVector3d grid_point,TorusIter current_torus){
...
bool molecular_surface::is_inside_Visibility_Sphere_old(TorusIter current_torus,CVector3d point_pos){
...
bool molecular_surface::is_inside_Visibility_Sphere(TorusIter current_torus,CVector3d point_pos){
	double check = (point_pos - current_torus->vs_sphere_center).LengthSquared() - pow(current_torus->vs_sphere_r,2);
...
bool molecular_surface::validate_atom_intersection(AtomIter atom,CVector3d pos){
...
void molecular_surface::process_each_atom_newer(AtomIter current_atom){
...
void molecular_surface::process_each_concave_sphere_newer(Concave_SphereIter current_concave){
...
void molecular_surface::process_each_torus_newer(TorusIter current_torus){
...
bool molecular_surface::is_inside_accessible_torus_old(TorusIter current_torus,grid_pointIter current_point){
...
bool molecular_surface::is_inside_accessible_torus(TorusIter current_torus,grid_pointIter current_point){
...
bool molecular_surface::is_inside_accessible_torus_with_output(TorusIter current_torus,grid_pointIter current_point){
...
void molecular_surface::check_regular(){
...
void molecular_surface::check_the_grid_points(){
	for(int i=0;i<m_grid_points.size();i++){
		grid_pointIter current_point = m_grid_points[i];
		if(current_point->is_processed)
			continue;
...
void molecular_surface::process_others_ray_counting(){
	for(int i=0;i<m_grid_points.size();i++){
		grid_pointIter current_point = m_grid_points[i];
		if(current_point->is_processed)
			continue;
...
void molecular_surface::process_ray_racing_each_point(grid_pointIter current_point){
...
void molecular_surface::process_each_augmented_atom(AtomIter current_atom){
	//decide which gird points are inside this atom sphere
	int tmp_min_x = int(floor((current_atom->center[0] - current_atom->r - m_probe_radius - m_min_x)/m_x_step));
	int tmp_min_y = int(floor((current_atom->center[1] - current_atom->r - m_probe_radius - m_min_y)/m_y_step));
	int tmp_min_z = int(floor((current_atom->center[2] - current_atom->r - m_probe_radius - m_min_z)/m_z_step));
...
int molecular_surface::compute_surface_intersection_num(grid_pointIter current_gridpoint,grid_pointIter processed_ngb){
...
int molecular_surface::compute_intersection_single_atom(grid_pointIter point_inside, grid_pointIter point_outside,AtomIter atom,std::vector<double> &t){
	/***
...
int molecular_surface::compute_atom_intersection_num(grid_pointIter point_inside, grid_pointIter point_outside){
	int num_intersection = 0;
...
int molecular_surface::compute_torus_intersection_num(grid_pointIter point_inside,grid_pointIter point_outside){
	int num_intersection = 0;
...
bool molecular_surface::solveQuartic(double a, double b, double c, double d, double e, double *root){
	//switch to the Jenkins-Traub algorithm
	solve_quartic_equation(a,b,c,d,e,root);
	return true;
}

int molecular_surface::compute_line_torus_intersection(TorusIter current_torus, CVector3d grid_pos_interior,CVector3d grid_pos_outside,CVector3d pos_interior, 
...
int molecular_surface::compute_concave_intersection_num(grid_pointIter point_inside,grid_pointIter point_outside){
...
int molecular_surface::process_grid_intersection_new(grid_edgeIter current_edge){
...
void molecular_surface::process_grid_atom_intersection_new(grid_pointIter point_inside, grid_pointIter point_outside,std::vector<CVector3d> &intersect_points,
...
void molecular_surface::process_grid_torus_intersection_new(grid_pointIter point_inside,grid_pointIter point_outside,TorusIter current_torus, std::vector<CVector3d> &intersect_points,
										 std::vector<CVector3d> &intersect_normal,std::vector<int> &intersect_type,std::vector<cell_unit> &intersect_cell){
...
void molecular_surface::process_grid_edge_concave_new(grid_pointIter point_inside,grid_pointIter point_outside,std::vector<CVector3d> &intersect_points,
...
void molecular_surface::clean_memory(){
    //clean the grids
  //cout<<"@@@@@@@@@@@@"<<endl;
...
bool molecular_surface::is_inside_torus(TorusIter current_torus,grid_pointIter current_point){
...
bool molecular_surface::is_inside_torus_new(TorusIter current_torus,grid_pointIter current_point){
//check if it is within the visibility sphere
	double check2 = (current_point->grid_pos - current_torus->vs_sphere_center).Length() - current_torus->vs_sphere_r;
	if(check2>0)
		return false;
...
bool molecular_surface::is_outside_concave_sphere(TorusIter current_torus,CVector3d current_point){
...
bool molecular_surface::search_the_processed_neighbor(grid_pointIter current_point, grid_pointIter &processed_neighbor,int &found_k,int &mode){
...
bool molecular_surface::solveQuadraticOther(double a, double b, double c, double &root){
...
bool molecular_surface::solveQuadratic(double a, double b, double c, double &root){
	if(a == 0.0 || fabs(a/b) < 1.0e-6)
	{
		if(fabs(b) < 1.0e-6) 
			return false;
...
bool molecular_surface::solveCubic(double a, double b, double c, double d, double &root){
	if(a == 0.0 || fabs(a/b) < 1.0e-6)
...
double molecular_surface::f_value(double a ,double b, double c, double d, double e, double x){
	return (a*pow(x,4) + b*pow(x,3) + c*pow(x,2) + d*x + e);
}

double molecular_surface::f_derivative_value(double a ,double b, double c, double d, double e, double x){
	return (4*a*pow(x,3) + 3*b*pow(x,2) + 2*c*x + d);
}

bool molecular_surface::solve_newton_method(double a ,double b, double c, double d, double e,double x_0, double &t){
...
bool molecular_surface::is_inside_spherical_triangle(Concave_SphereIter current_concave,CVector3d test_point){
...
void molecular_surface::accum_surface_area(grid_pointIter point1,grid_pointIter point2,CVector3d intersect_normal){
...
int molecular_surface::partition_area(int a, int b, int c, int block_size){
...
int molecular_surface::process_each_grid_edge(grid_pointIter point_inside, grid_pointIter point_outside,std::vector<CVector3d> &intersect_points,
			   std::vector<CVector3d> &intersect_normal,std::vector<int> &intersect_type,std::vector<cell_unit> &intersect_cell){
...
void molecular_surface::write_intersections(intersection_info& intersection, int block_x, int block_y, int block_z, int block_size){
    for(int i=0; i<m_intersection_points.size(); ++i){
        grid_point* gp_in = m_intersection_interior[i];
        grid_point* gp_out = m_intersection_outside[i];

        if((gp_in->grid_x==block_x*block_size+m_x_num-1 && gp_out->grid_x==block_x*block_size+m_x_num-1) 
            || (gp_in->grid_y==block_y*block_size+m_y_num-1 && gp_out->grid_y==block_y*block_size+m_y_num-1)
            || (gp_in->grid_z==block_z*block_size+m_z_num-1 && gp_out->grid_z==block_z*block_size+m_z_num-1))
            continue;

        CVector3d gv_in(gp_in->grid_x, gp_in->grid_y, gp_in->grid_z);
        CVector3d gv_out(gp_out->grid_x, gp_out->grid_y, gp_out->grid_z);
        intersection.m_grid_inside.push_back(gv_in);
        intersection.m_grid_outside.push_back(gv_out);

        intersection.m_intersection.push_back(m_intersection_points[i]);
        intersection.m_normal.push_back(m_intersection_normals[i]);

        vector<int> adj;
        if(m_intersection_types[i] == 0){
            AtomIter ai = m_intersection_cell[i].current_atom;
            adj.push_back(m_local_to_global_atom_idx[ai->index]);
        }
        else if(m_intersection_types[i] == 1){
            TorusIter ti = m_intersection_cell[i].current_torus;
            adj.push_back(m_local_to_global_atom_idx[ti->atom_i->index]);
            adj.push_back(m_local_to_global_atom_idx[ti->atom_j->index]);
        }
        else if(m_intersection_types[i] == 2){
            Concave_SphereIter ci = m_intersection_cell[i].current_concave;
            adj.push_back(m_local_to_global_atom_idx[ci->index_i]);
            adj.push_back(m_local_to_global_atom_idx[ci->index_j]);
            adj.push_back(m_local_to_global_atom_idx[ci->index_k]);
        }
        intersection.m_adj_atoms.push_back(adj);
    }
}

void molecular_surface::test_intersection(){
...
CVector3d molecular_surface::grid_pos(int X,int Y,int Z){
	return CVector3d(X * m_x_step + m_min_x,Y * m_y_step + m_min_y,Z * m_z_step + m_min_z);
}

int molecular_surface::grid_index(int X,int Y, int Z){

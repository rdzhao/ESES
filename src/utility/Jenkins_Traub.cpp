	...
	if (op[0] != 0){
		...
		while (N >= 1){ // Main loop
			...
			for (jj = 1; jj <= 40; jj++){
				...
				Fxshfr_ak1(40*jj, &NZ, sr, bnd, K, N, p, NN, qp, u, &lzi, &lzr, &szi, &szr);
			...
			if (jj > 40) {
				cout << "\nFailure. No convergence after 40 shifts. Program terminated.\n";
				*Degree -= N;
				break;
			} // End if (jj > 20)
...
void solve_quartic_equation(double a,double b,double c,double d,double e,double *root){
	int Degree = 4;
